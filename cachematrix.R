...
## returns a list containing 4 functions which is similar to the example given
makeCacheMatrix <- function(x = matrix()) {
  m <- NULL
  set <- function(y) { #allows you to change the matrix stored
    x <<- y
    m <<- NULL
  }
  get <- function() x #allows you to call the matrix stored
  setinverse <- function(inverse) m <<- inverse #this allows you to cache a matrix with the cache funciton below
